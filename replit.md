...
- **Hero Fragment**:
    ...
    - **Ultra-Aggressive Hardcoding (Aug 16, 2025)**: All layout-related CSS variables replaced with fixed values - spacing (10rem, 4rem, 3rem, 1.5rem, etc.), typography (3.5rem, 1.25rem, 1.125rem, etc.), font weights (900, 600, 400), and border radius (0.5rem, 0.25rem). Fragment now renders with zero dependency on external CSS variables.
    - **Hybrid Variable Restoration (Aug 16, 2025)**: JavaScript restores 50+ Liferay theme CSS variables on hero element after hardcoded styles render instantly. Provides both immediate LCP performance (no blocking) and full dynamic theming capability. Variables include complete color system, typography, spacing, buttons, borders, and shadows.
