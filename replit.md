...
- **Hero Fragment**:
    ...
    - **Performance Ultra-Optimized**: Targeting 90+ Lighthouse score with aggressive LCP optimization (sub-2.5s), CLS elimination (<0.1), fully inline CSS for immediate rendering, eager image loading, complete mobile responsiveness, and zero debugging artifacts. Hero fragment is completely self-contained with ALL styles (critical and non-critical) inline within the HTML file, hardcoded colors (no CSS custom properties), and embedded Google Fonts for maximum performance and zero external CSS dependencies including main.css and clay.css.
- **Footer Fragment**:
